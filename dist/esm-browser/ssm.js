...
var ACK_TYPE = 3;
var INPUT_TYPE = 1; //const OUTPUT_TYPE = 0;
...
function parseUuid(buf) {
  ...
  return "".concat(part1, "-").concat(part2, "-").concat(part3, "-").concat(part4, "-").concat(part5);
}
...
function longToByteArray(_long) {
  var byteArray = [0, 0, 0, 0, 0, 0, 0, 0];

  for (var index = byteArray.length - 1; index >= 0; index--) {
    var _byte = _long & 0xff;

    byteArray[index] = _byte;
    _long = (_long - _byte) / 256;
  }

  return byteArray;
}

function intToByteArray(_int) {
  var byteArray = [0, 0, 0, 0];

  for (var index = byteArray.length - 1; index >= 0; index--) {
    var _byte2 = _int & 0xff;

    byteArray[index] = _byte2;
    _int = (_int - _byte2) / 256;
  }
...
var ssm = {
  ...
  buildInitMessage: function buildInitMessage(options) {
    var payload = {
      cols: options.cols,
      rows: options.rows
    };
  ...
  buildAcknowledge: function buildAcknowledge(messageType, sequenceNumber, messageID) {
    var payload = {
  ...
  buildInputMessage: function buildInputMessage(text, sequenceNumber) {
